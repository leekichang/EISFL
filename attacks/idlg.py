...
class iDLG(Attacker):
    def __init__(self, args):
        ...
        self.optimizer = None
        self.iteration = 300
        self.criterion = dlg_utils.cross_entropy_for_onehot
        pass
    
    def attack(self, old_model, new_model, gnd_labels=None):
        ...
        if gnd_labels == None: # or self.args.n_user == 1:
            gnd_label = torch.argmin(torch.sum(original_dy_dx[-2], dim=-1), dim=-1).detach().reshape((1,)).requires_grad_(False)
            print(f"Evaluated ground truth label is {gnd_label}")
            gnd_label = dlg_utils.label_to_onehot(gnd_label, cfg.N_CLASS[self.args.dataset])
            ...
            # gnd_labels = gnd_labels.clone()
            # for label in gnd_labels:
            #     label = dlg_utils.label_to_onehot(label, cfg.N_CLASS[self.args.dataset])
            # label = gnd_labels
            gnd_label = dlg_utils.label_to_onehot(gnd_labels, cfg.N_CLASS[self.args.dataset]).to(self.device)
            dummy_data = torch.randn(cfg.IMGSIZE[self.args.dataset]).to(self.device).requires_grad_(True)
            optimizer = optim.LBFGS([dummy_data])
        ...
        for iters in range(self.iteration):
            def closure():
                ...
                dummy_pred = old_model(dummy_data).to(self.device)
                dummy_loss = self.criterion(dummy_pred, gnd_label)
